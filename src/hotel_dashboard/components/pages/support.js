        ...
        <Layout>
          <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-6">
            <div className="text-center">
              ...
              />
              <p className="text-gray-600 font-medium text-lg">Broccoli is crunching your support...</p>
            </div>
          </div>
        </Layout>
      );
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8">
          ...
          {activeTab === 'new' && (
            <div className="bg-white rounded-xl shadow-md p-6 mb-6">
              <h2 className="text-xl font-semibold text-green-800 mb-4">Create New Ticket</h2>
              <form onSubmit={submitNewTicket}>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 border border-green-200 rounded-lg focus:ring-2 focus:ring-green-500 resize-none"
                    required
                  />
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-green-700 mb-2">Attachments (Optional)</label>
                  <input
                    type="file"
                    multiple
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 transition-colors font-semibold text-sm disabled:opacity-50"
                >
            ...
            <div className="space-y-6">
              {filteredTickets.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-green-600 mb-4">Start a new conversation to get help.</p>
                  <button
                ...
                filteredTickets.map(ticket => (
                  <div key={ticket.id} className="bg-white rounded-xl shadow-md overflow-hidden">
                    <div className="p-6 border-b border-green-100 flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-green-700 mb-2">{ticket.category}</p>
                        <p className="text-sm text-gray-600 mb-2">ID: #{ticket.id} • Last Updated: {new Date(ticket.updated_at).toLocaleDateString()}</p>
                        {ticket.messages && ticket.messages.length > 1 && (
                          <p className="text-xs text-green-600">Replies: {ticket.messages.length - 1}</p>
                        )}
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-medium text-green-800">Priority: {ticket.priority || 'Medium'}</p>
                      </div>
                    </div>
                            ...
                            <div className="max-w-xs">
                              ...
                              <div className="flex space-x-2 mt-2">
                                ...
                                <button
                                  ...
                                  className="px-4 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50"
                                >
                                  Send Reply
                                </button>
                              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={idx} className="border-l-4 border-green-400 pl-4">
                  <h4 className="font-medium text-green-800 mb-1">{faq.q}</h4>
          ...
          {safeTickets.filter(t => t.status === 'closed').length > 0 && (
            <div className="mt-8 text-center">
              <h3 className="text-lg font-semibold text-green-800 mb-2">How was your last experience?</h3>
              <div className="flex justify-center space-x-2">
                {[1, 2, 3, 4, 5].map(star => (
                  <button key={star} className="text-2xl text-yellow-400 hover:text-yellow-500">⭐</button>
                ))}
              </div>
              <p className="text-sm text-green-600 mt-2">Rate us to help improve service!</p>
            </div>
          )}
        </div>
      </div>
