...
//     try {
//       const data = await fetch(`http://127.0.0.1:5000/api/admin/users/${id}`, {
...
//     try {
//       const data = await fetch(`http://127.0.0.1:5000/api/admin/orders?user_id=${id}`, {
//         headers: { Authorization: `Bearer ${token}` }
//       });
...
//     try {
//       const data = await fetch(`http://127.0.0.1:5000/api/admin/bills?user_id=${id}`, {
//         headers: { Authorization: `Bearer ${token}` }
//       });
//       if (!data.ok) throw new Error('Failed to fetch bills');
//       const billsData = await data.json();
...
//     try {
//       const data = await fetch('http://127.0.0.1:5000/api/products', {
//         headers: { Authorization: `Bearer ${token}` }
//       });
...
//       };
//       const orderRes = await fetch('http://127.0.0.1:5000/api/admin/orders', {
//         method: 'POST',
...
//       };
//       const res = await fetch(`http://127.0.0.1:5000/api/admin/bills/${editingBill.id}`, {
//         method: 'PUT',
//         headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
...
//       ...item,
//       price_per_unit: parseFloat(item.price_at_order || 0)
//     })) || [];
...
//           <td>${item.quantity} ${item.unit_type || 'unit'}</td>
...
//           <span className="text-gray-600">Loading hotel details…</span>
//         </div>
//       </Layout>
//     );
//   }
...
//             <button onClick={() => navigate('/admin/hotels')} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded">
//               Back to Hotels
//             </button>
//           </div>
//         </div>
...
//     return (
//       <Layout>
//         <div className="p-6 text-center">Hotel not found</div>
//       </Layout>
//     );
//   }
...
//           <div>
//             <h1 className="text-3xl font-bold text-gray-900">{safe(hotel.hotel_name)} Dashboard</h1>
//             <p className="text-gray-600 mt-2">Manage details and orders for this hotel</p>
//           </div>
//           <button
...
//               <div><p className="text-sm text-gray-600">Username</p><p className="font-semibold">{safe(hotel.username)}</p></div>
//               <div><p className="text-sm text-gray-600">Email</p><p className="font-semibold">{safe(hotel.email)}</p></div>
//               <div><p className="text-sm text-gray-600">Phone</p><p className="font-semibold">{safe(hotel.phone)}</p></div>
//               <div><p className="text-sm text-gray-600">Address</p><p className="font-semibold">{safe(hotel.address)}</p></div>
//               <div><p className="text-sm text-gray-600">Created At</p><p className="font-semibold">{formatDate(hotel.created_at)}</p></div>
//               <div><p className="text-sm text-gray-600">Last Login</p><p className="font-semibold">{formatDate(hotel.last_login)}</p></div>
//             </div>
//           </div>
//         </div>
...
//                       <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(o.order_date)}</td>
//                       <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(o.delivery_date)}</td>
//                       <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(o.total_amount)}</td>
...
//                         <button className="text-blue-600 hover:text-blue-900 text-xs underline">View Details</button>
//                       </td>
//                     </tr>
//                   ))
//                 )}
...
//                         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">#{b.order_id}</td>
//                         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(amount)}</td>
//                         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(b.bill_date)}</td>
//                         <td className="px-6 py-4 whitespace-nowrap">
//                           <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
...
//                           </span>
//                         </td>
//                         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{b.payment_method || 'N/A'}</td>
//                         <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
...
//                   <h3 className="text-xl font-bold text-gray-900">New Order for {hotel.hotel_name}</h3>
...
//                           <img src={p.image_url} alt={p.name} className="w-full h-20 object-cover rounded mb-2" />
...
//               <div className="p-6">
//                 <div className="flex justify-between items-center mb-6">
//                   <h3 className="text-xl font-bold text-gray-900">Edit Bill #{editingBill.id}</h3>
...
const HotelDetail = () => {
      ...
      <Layout>
        <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 font-medium text-lg">Loading hotel details…</p>
          </div>
        </div>
      </Layout>
    );
  ...
  if (error) {
    return (
      <Layout>
        <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-8">
          <div className={`bg-red-50 border-2 border-red-200 rounded-2xl p-6 max-w-md w-full shadow-lg`}>
  ...
  if (!hotel) {
    return (
      <Layout>
        <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-8">
          <div className="text-center text-gray-600 font-medium">Hotel not found</div>
        </div>
      </Layout>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 p-8 w-full">
        ...
        <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 text-lg font-medium">Manage details and orders for this hotel</p>
          </div>
          <button
            onClick={() => navigate('/admin/hotels')}
        ...
        <div className="bg-white rounded-2xl shadow-lg border-2 border-green-100 overflow-hidden mb-10">
          <div className="px-6 py-5 bg-gradient-to-r from-green-50 to-emerald-50 border-b-2 border-green-100">
            <h2 className="text-xl font-bold text-green-800">Hotel Details</h2>
          </div>
          <div className="p-6">
            <div className="flex flex-col md:flex-row gap-8">
              <div className="flex-shrink-0">
                {safe(hotel.hotel_image) ? (
                  ...
                  <div className="w-64 h-48 bg-green-50 rounded-xl flex items-center justify-center border-2 border-green-200">
                    <span className="text-4xl">🏨</span>
                  </div>
                )}
              </div>
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><p className="text-sm font-medium text-gray-600">Username</p><p className="font-semibold text-green-800">{safe(hotel.username)}</p></div>
                <div><p className="text-sm font-medium text-gray-600">Email</p><p className="font-semibold text-green-800">{safe(hotel.email)}</p></div>
                <div><p className="text-sm font-medium text-gray-600">Phone</p><p className="font-semibold text-green-800">{safe(hotel.phone)}</p></div>
                <div><p className="text-sm font-medium text-gray-600">Address</p><p className="font-semibold text-green-800">{safe(hotel.address)}</p></div>
                <div><p className="text-sm font-medium text-gray-600">Created At</p><p className="font-semibold text-green-800">{formatDate(hotel.created_at)}</p></div>
                <div><p className="text-sm font-medium text-gray-600">Last Login</p><p className="font-semibold text-green-800">{formatDate(hotel.last_login)}</p></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="bg-green-50/50">
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">ID</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Date</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Delivery</th>
                ...
                {orders.length === 0 ? (
                  ...
                  orders.map(o => (
                    <tr key={o.id} className="hover:bg-green-50/30 transition-colors">
                      <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">#{o.id}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{formatDate(o.order_date)}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{formatDate(o.delivery_date)}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{formatCurrency(o.total_amount)}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
            ...
            <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-green-100">
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => setSelectedOrder(null)} className="p-2 hover:bg-green-50 rounded-xl transition-colors text-gray-400 hover:text-gray-600">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-xl border-2 border-green-100">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-xl font-bold text-green-800">{safe(hotel?.hotel_name)}</h4>
                        <p className="text-gray-600">Order #{selectedOrder.id}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-3xl font-bold text-emerald-700">{formatCurrency(selectedOrder.total_amount)}</p>
                        <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${
                          selectedOrder.status === 'delivered' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                        }`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-lg font-semibold text-gray-900">{formatDate(selectedOrder.order_date || selectedOrder.created_at)}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Delivery Date</p>
                        <p className="text-lg font-semibold text-gray-900">{formatDate(selectedOrder.delivery_date)}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <p className="text-sm font-medium text-gray-600">Contact Info</p>
                        <p className="text-lg font-semibold text-gray-900">{safe(hotel?.email)}</p>
                        <p className="text-gray-600">{safe(hotel?.phone)}</p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border-2 border-green-100 overflow-hidden mb-10">
          <div className="px-6 py-5 bg-gradient-to-r from-green-50 to-emerald-50 border-b-2 border-green-100">
            <h2 className="text-xl font-bold text-green-800">Bills</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="bg-green-50/50">
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">ID</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Order ID</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Amount</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Date</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Payment Method</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-green-50">
                  ...
                  bills.map(b => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-green-50/30 transition-colors">
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">#{b.id}</td>
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">#{b.order_id}</td>
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{formatCurrency(amount)}</td>
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{formatDate(b.bill_date)}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            b.paid ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                          }`}>
                            {b.paid ? 'Paid' : 'Pending'}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">{b.payment_method || 'N/A'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            <button
                              onClick={() => viewBill(b)}
